    ...
    try:
        with YoutubeDL(ydl_opts) as ydl:
            ...
            for e in info.get('entries',[]):
                if not e: continue
                res.append({'id':e.get('id'),'title':e.get('title'),'artist':e.get('uploader') or 'YouTube','thumb':f"https://i.ytimg.com/vi/{e.get('id')}/mqdefault.jpg"})
            return jsonify(res)
    except:
        return jsonify([])
...
def play():
    ...
    # Tenta Piped - mais rapido pra musica curta
    try:
        r=requests.get(f"https://pipedapi.kavin.rocks/streams/{vid}",timeout=12)
        j=r.json()
    ...
    for api in ["https://api.cobalt.tools","https://co.wuk.sh/api/json"]:
        try:
            ...
            j=r.json()
            if j.get('url'):
                return jsonify({'audio': j.get('url')})
        except:
            continue
    # Fallback yt-dlp
    try:
        ydl_opts={'format':'bestaudio/best','quiet':True,'extractor_args':{'youtube':{'player_client':['android']}}}
        with YoutubeDL(ydl_opts) as ydl:
        ...
        pass
    return jsonify({'error':'Tente outra mais curta'})
