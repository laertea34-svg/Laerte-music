...
def search():
    ...
    # Usa Piped para buscar também, não bloqueia
    try:
        r = requests.get(f"https://pipedapi.kavin.rocks/search?q={q}&filter=music_songs", timeout=10)
        data = r.json()
        res = []
        for item in data.get('items', [])[:8]:
            res.append({
                'id': item.get('url','').split('v=')[-1].split('&')[0].replace('/watch?v=',''),
                'title': item.get('title'),
                'artist': item.get('uploaderName') or 'YouTube',
                'thumb': item.get('thumbnail') or f"https://i.ytimg.com/vi/{item.get('url','').split('v=')[-1]}/mqdefault.jpg"
            })
        if res:
            return jsonify(res)
    except: pass
...
def play():
    ...
    # PLANO B: Pega áudio via Piped (não pede login)
    try:
        r = requests.get(f"https://pipedapi.kavin.rocks/streams/{vid}", timeout=10)
        j = r.json()
        # pega melhor áudio
        audio_streams = j.get('audioStreams', [])
        if audio_streams:
            # ordena por bitrate
        ...
        # fallback hls
        if j.get('hls'):
            return jsonify({'audio': j.get('hls')})
    except Exception as e:
        print("Piped erro:", e)

    # ultimo fallback yt-dlp android
    try:
        from yt_dlp import YoutubeDL
        url=f"https://www.youtube.com/watch?v={vid}"
        ydl_opts={'format':'bestaudio[ext=m4a]/bestaudio/best','quiet':True,'extractor_args':{'youtube':{'player_client':['android_music']}}}
        with YoutubeDL(ydl_opts) as ydl:
            info=ydl.extract_info(url,download=False)
            return jsonify({'audio':info.get('url')})
    except Exception as e:
        return jsonify({'error': f"Erro YouTube: {str(e)[:150]}"})
